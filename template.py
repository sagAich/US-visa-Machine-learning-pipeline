...
list_of_files = [
    ...
    '''
    here us_visa is not directly mentioned because we are using f string and project_name
variable is assigned to us_visa. so that if we change the project name we just need to change the value of 
      project_name variable and rest of the code will work without any change.
      '''
...
''' 
without exist_ok = true, suppose you run os.makedirs("us_visa/components") python will see that folder already exists hence it will throw error FileExistsError: [Errno 17] File exists: 'us_visa'.
...
after the first iteration us_visa/ folder is created, then If us_visa/ already exists, why does Python create components/ inside it instead of creating another separate us_visa/ folder?
...
'''
not os.path.exists(filepath): The file does not exist at the specified path. os.path.getsize(filepath) == 0: The file exists but has a size of 0 bytes (it is empty).

why not use filename instead of filepath?

because filename contains ONLY the file name. It does NOT contain the folder location. suppose we do open(filename, "w") this becomes open("__init__.py", "w"), then Python creates the file in the CURRENT directory. NOT inside us_visa/. so we use filepath which results in open("us_visa/__init__.py", "w"). Now the file is created INSIDE the folder.

what is the use of the condition "if path.os.path.getsize(filepath) == 0" simply we could have checked The file does not exist at the specified path, through "if not os.path.exists(filepath)" statement ?
